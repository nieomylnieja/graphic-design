    ...
    struct PositionData {
        ...
        public:
            ...
            bool isPiecePinned(const PositionData &pos, const Square &from, const Square &to) {
                ...
                king_itr = std::find_if(position.squares.begin(), position.squares.end(),
                                        PieceSquarePredicate(my_king));
        ...
        public:
            ...
            bool operator()(const Square &s) {
                ...
                bool validMove = rule->checkLegalMovement(position, s,
                                                          ply.toSquare()); // is rook from 's' to 'e5' a chess move?
...
void pgn::Position::update(pgn::Ply &ply) {
    ...
    if (ply.isCapture() || ply.piece() == Piece::Pawn() || ply.piece() == Piece::WhitePawn() ||
        ply.piece() == Piece::BlackPawn()) {
        hdata->halfmoveClock = 0;
    } else {
        hdata->halfmoveClock++;
    }
